...
// --- JAVASCRIPT THÊM CHO PHẦN BÀI TẬP VỀ NHÀ 1 ---

// 1. Xử lý đếm ký tự Họ và Tên (Tối đa 50)
const fullnameInput = document.getElementById('fullname');
const nameCountSpan = document.getElementById('nameCount');

fullnameInput.addEventListener('input', () => {
    const len = fullnameInput.value.length;
    nameCountSpan.textContent = `${len}/50`;
});
...
toggleBtn.addEventListener('click', () => {
    ...
    // Đổi icon tương ứng (mở mắt / nhắm mắt)
    toggleBtn.textContent = type === 'password' ? '👁' : '🙈';
});


// 3. Xử lý Thanh mức độ mạnh mật khẩu
...
passInput.addEventListener('input', () => {
    ...
    // Các tiêu chí cộng điểm độ mạnh
    if (val.length >= 6) strengthScore += 1; // Độ dài cơ bản
    if (val.length >= 8 && /[A-Z]/.test(val) && /[0-9]/.test(val)) strengthScore += 1; // Có hoa + có số
    if (val.length >= 8 && /[^A-Za-z0-9]/.test(val)) strengthScore += 1; // Có ký tự đặc biệt (!@#$...)

    // Reset class của thanh màu
    strengthBar.className = 'strength-bar'; 

    if (val.length === 0) {
        strengthBar.style.width = '0';
        strengthText.textContent = '';
    } else if (strengthScore === 1 || strengthScore === 0) {
